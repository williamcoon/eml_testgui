...
class MainGUI(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.home_btn = make_button("Home")
    ...
    def get_recipe_widget(self):
        ...
        self.z_height = LabeledInputGrid("Z Height", LENGTH_UNIT)
        ...
        self.recipe_inputs.append(self.z_height)
        ...
        self.saved_recipes.addItem("No Recipes Found")

        self.load_recipe_btn = make_button("Load Recipe")
        self.save_recipe_btn = make_button("Save Recipe")
        self.select_recipe_btn = make_button("Select Current Recipe")
        self.delete_recipe_btn = make_button("Delete Recipe")
        self.create_recipe_btn = make_button("Create New Recipe")

        recipe_controls_layout.addWidget(self.saved_recipes)
        recipe_controls_layout.addWidget(self.load_recipe_btn)
        recipe_controls_layout.addWidget(self.select_recipe_btn)
        ...
        recipe_layout.addWidget(self.delete_recipe_btn)
        recipe_layout.addWidget(self.create_recipe_btn)
    ...
    def setup_qt_connections(self):
        ...
        self.delete_recipe_btn.clicked.connect(self.delete_recipe_btn_clicked)
        self.select_recipe_btn.clicked.connect(self.select_recipe_btn_clicked)
        self.create_recipe_btn.clicked.connect(self.create_recipe_btn_clicked)
    ...
    def home_btn_clicked(self):
        self.sender.send_message(HummingBirdMessages.HOME.value)

    def create_recipe_btn_clicked(self):
        recipe = self.sender.send_message(HummingBirdMessages.CREATE_RECIPE.value)
        self.update_recipe_display(recipe)

    def delete_recipe_btn_clicked(self):
        message = HummingBirdMessages.DELETE_RECIPE.value
        message += self.recipe_name_txt.text()
        self.sender.send_message(message)
        self.request_recipe_list()

    def save_recipe_btn_clicked(self):
        message = HummingBirdMessages.UPDATE_RECIPE.value
        message += self.recipe_name_txt.text()
        for recipe_input in self.recipe_inputs:
            message += f",{recipe_input.input_txt.text()}"
        self.sender.send_message(message)
        self.sender.send_message(HummingBirdMessages.SAVE_RECIPE.value)
